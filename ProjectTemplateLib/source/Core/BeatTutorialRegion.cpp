...
BeatTutorialRegion::BeatTutorialRegion(sf::RenderWindow& window) :
	m_gui(window)
{
	InitGui();
}

bool BeatTutorialRegion::handleEvent(sf::Int64 elapsedTime, const sf::Event& event)
{
	return m_gui.handleEvent(event);
}

tgui::Gui& BeatTutorialRegion::GetGui()
{
	return m_gui;
}

void BeatTutorialRegion::InitGui()
{
...
void BeatTutorialRegion::SetFinalShip(std::shared_ptr<Ship> finalShip)
{
	InitGui();
